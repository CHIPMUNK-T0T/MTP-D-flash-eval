...
if [ "$MODE" = "--dry-run" ]; then
  bash "$ROOT_DIR/benchmark_mtp.sh" --dry-run
  bash "$ROOT_DIR/benchmark_dflash.sh" --dry-run
  bash "$ROOT_DIR/benchmark_ollama.sh" --dry-run
  exit 0
fi

if [ "$MODE" != "--summary-only" ]; then
  bash "$ROOT_DIR/benchmark_mtp.sh"
  bash "$ROOT_DIR/benchmark_dflash.sh"
  bash "$ROOT_DIR/benchmark_ollama.sh"
fi
...
with summary_md.open("w", encoding="utf-8") as f:
    ...
    f.write("Generated after `benchmark_all.sh` completes.\n\n")
