...
CONFIGS=(
  "Base-FP8|FP8 baseline|fp8|no|0|qwen35-4b-fp8.sh|RedHatAI/Qwen3.5-4B-FP8-dynamic"
  "DFlash-FP8-n15|FP8 D-Flash n=15|fp8|yes|15|qwen35-4b-fp8-dflash-n15.sh|RedHatAI/Qwen3.5-4B-FP8-dynamic"
)
...
config_id,label,quantization,dflash,num_speculative_tokens,workload,max_tokens,run_index,elapsed_ms,completion_tokens,tok_per_s,status
EOF

  cat > "$SUMMARY_CSV" <<'EOF'
config_id,label,quantization,dflash,num_speculative_tokens,workload,max_tokens,max_num_batched_tokens,status,median_elapsed_ms,median_tok_per_s,draft_tokens,accepted_tokens,acceptance_rate,startup_seconds,notes
EOF
}
...
acquire_lock() {
  ...
  if ! mkdir "$LOCK_DIR" 2>/dev/null; then
    echo "another benchmark_dflash_4b.sh run appears to be active: $LOCK_DIR" >&2
    exit 1
  fi
...
run_workload() {
  ...
  if ! curl -s --max-time 420 http://localhost:8000/v1/chat/completions \
    ...
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
      "$config_id" "$label" "$quant" "$dflash" "$spec_n" "$workload" "$max_tokens" \
      "$MAX_NUM_BATCHED_TOKENS" "warmup_failed" "" "" "" "" "" "$startup_seconds" "warmup_curl_failed" >>"$SUMMARY_CSV"
    return 0
  fi
...
PY
  ...
  if [ "$run_status" != "ok" ]; then
    printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
      "$config_id" "$label" "$quant" "$dflash" "$spec_n" "$workload" "$max_tokens" \
      "$MAX_NUM_BATCHED_TOKENS" "$run_status" "" "" "" "" "" "$startup_seconds" "$run_notes" >>"$SUMMARY_CSV"
    return 0
  fi
...
PY
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "$config_id" "$label" "$quant" "$dflash" "$spec_n" "$workload" "$max_tokens" \
    "$MAX_NUM_BATCHED_TOKENS" "ok" "$median_elapsed" "$median_tps" "$draft_delta" \
...
baseline = {}
for row in rows:
    if row["status"] == "ok" and row["config_id"] == "Base-FP8":
...
with open(summary_md, "w", encoding="utf-8") as out:
    out.write("# Qwen3.5-4B D-Flash Benchmark Summary\n\n")
    ...
    for workload in ("medium", "long"):
        ...
        out.write("| Config | D-Flash | n | tok/s median | Speedup vs Base-FP8 | Acceptance | Status | Notes |\n")
        out.write("| --- | --- | ---: | ---: | ---: | ---: | --- | --- |\n")
        for row in grouped.get(workload, []):
            ...
            out.write(
                f"| {row['config_id']} {row['label']} | {row['dflash']} | "
                f"{row['num_speculative_tokens']} | {tok} | {speedup} | "
                f"{acceptance} | {row['status']} | {row['notes']} |\n"
            )
        out.write("\n")
PY
}
...
with review_md.open("w", encoding="utf-8") as out:
    out.write("# Qwen3.5-4B D-Flash Result Review\n\n")
    ...
    out.write("\n## Quality scan\n\n")
    if quality_issues:
        out.write("Potential repetition loops were detected in these response sets:\n\n")
...
if [ "$DRY_RUN" = "--dry-run" ]; then
  ...
  for config in "${CONFIGS[@]}"; do
    IFS='|' read -r config_id label quant dflash spec_n script_name model <<<"$config"
    echo "config=$config_id script=$script_name dflash=$dflash n=$spec_n model=$model"
  done
  exit 0
fi
...
for config in "${CONFIGS[@]}"; do
  ...
  if ! MAX_NUM_BATCHED_TOKENS="$MAX_NUM_BATCHED_TOKENS" \
    ...
    for workload in "${WORKLOADS[@]}"; do
      ...
      printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
        "$config_id" "$label" "$quant" "$dflash" "$spec_n" "$workload_name" "$max_tokens" \
  ...
  if ! startup_seconds="$(wait_for_startup "$startup_log")"; then
    for workload in "${WORKLOADS[@]}"; do
      ...
      printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
        "$config_id" "$label" "$quant" "$dflash" "$spec_n" "$workload_name" "$max_tokens" \
        "$MAX_NUM_BATCHED_TOKENS" "startup_failed" "" "" "" "" "" "$startup_seconds" "startup_failed" >>"$SUMMARY_CSV"
    done
    stop_server
    continue
  fi
  ...
  for workload in "${WORKLOADS[@]}"; do
    IFS='|' read -r workload_name max_tokens prompt_file <<<"$workload"
    run_workload "$config_id" "$label" "$quant" "$dflash" "$spec_n" "$model" \
      "$workload_name" "$max_tokens" "$prompt_file" "$startup_seconds"
  done
